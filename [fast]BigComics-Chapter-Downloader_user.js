...
(function () {
    ...
    const STABLE_INTERVAL  = 100;  // ms between stability checks (cheap check now, was 250)
    const AFTER_CLICK_WAIT = 350;  // ms to wait after a click before checking stability (was 600)
    ...
    // Cheap "did this canvas change?" check used during polling.
    // Drawing into a tiny 12x12 canvas + reading its pixels is orders of
    // magnitude faster than a full-resolution toDataURL('image/png') encode,
    // which is what was making the page-collection loop slow.
    let _sigCanvas = null, _sigCtx = null;
    function canvasSig(c) {
        try {
            if (!_sigCanvas) {
                _sigCanvas = document.createElement('canvas');
                _sigCanvas.width = 12;
                _sigCanvas.height = 12;
                _sigCtx = _sigCanvas.getContext('2d', { willReadFrequently: true });
            }
            _sigCtx.clearRect(0, 0, 12, 12);
            _sigCtx.drawImage(c, 0, 0, 12, 12);
            const d = _sigCtx.getImageData(0, 0, 12, 12).data;
            let h = 0;
            for (let i = 0; i < d.length; i += 4) {
                h = (h * 33 + d[i] + (d[i + 1] << 8) + (d[i + 2] << 16)) >>> 0;
            }
            return c.width + 'x' + c.height + '_' + h;
        } catch (e) {
            return 'err'; // tainted canvas: treat as unchanging, same as before
        }
    }
    ...
    function waitStableCanvases(minCount = 1, timeoutMs = 6000) {
        return new Promise((resolve, reject) => {
            ...
            const check = () => {
                ...
                const cs = getValidCanvases();
                const sig = cs.map(canvasSig).join(',');
        ...
        while (clicks < MAX_PAGES) {
            ...
            for (const c of canvases) {
                try {
                    ...
                    if (!collected.has(k)) {
                        // Store in the chosen output format. If output is PNG, the
                        // fingerprint we just computed IS the capture — reuse it
                        // instead of encoding the same canvas a second time.
                        collected.set(k, imageFormat === 'png' ? fingerprint : captureCanvas(c));
                        newThisRound++;
                    }
